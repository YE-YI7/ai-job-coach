...
阶段完成：当用户制定好投递计划后，主动推荐进入"模拟面试"阶段 ✅`,
  ...
  interview: AI_PERSONA_CORE + `【角色设定】
...
export async function POST(req: Request) {
  try {
    ...
    processPostChat({
      ...
      sessionId,
      stage: memoryStage,
      originalStage: stage || 'career_planning',
      userMessage,
      aiReply: reply,
    }).catch(err => {
...
async function processPostChat(params: {
  ...
  try {
    // 4. 异步更新待办规划——让 AI 自行判断是否需要生成/更新任务
...
已有任务列表：
${tasks.length > 0 ? tasks.map((t, i) => `${i}. [${t.is_completed ? '✅' : '⬜'}] ${t.title}`).join('\n') : '(暂无)'}
