...
import { UserStage, isValidStage } from "@/lib/stage";
...
export default function InterviewPanel({
  ...
  useEffect(() => {
    ...
    const scoreValues = scores ? Object.values(scores as Record<string, number>) : [];
  ...
  // 将 string 类型的 currentStage 安全转换为 UserStage | undefined，确保类型安全
  // 只允许 UserStage 的合法值，否则使用 undefined
  const safeStage: UserStage | undefined = isValidStage(currentStage) ? currentStage : undefined;
    ...
    >
      ...
      {latestSummary.summary?.scores && (
        <div className="grid grid-cols-2 gap-3 text-sm text-gray-700">
          {/* 将 scores 类型断言为 Record<string, number>，确保 value 是数字类型，可以安全渲染为 ReactNode */}
          {Object.entries(latestSummary.summary.scores as Record<string, number>).map(([key, value]) => (
