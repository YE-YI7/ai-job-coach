...
1. 如果存在 `yi_zhi_retrieve_knowledge`，先用目标岗位和 JD 检索内部知识库，静默校准岗位语言和常见筛选点；外部案例不得写进用户简历。
2. 前置最相关的经历和结果。
3. 补清问题场景、用户责任和协作边界。
4. 用具体动作替换“负责”“参与”等空泛表述。
5. 只使用用户确认的数据；没有数据时描述可观察影响。
6. 删除与目标岗位无关、重复或经不起追问的内容。
7. 自然覆盖 JD 术语，不堆砌关键词。
...
AI 建议只是可编辑草稿，不是最终事实。用户可以逐条改写建议内容；保留原文和 AI 初稿，用户修改后再次核对已确认事实、数字、责任边界和 JD 关键词，再生成可保存的目标岗位版本。未经这次复核，不把用户编辑稿标记为可投递。
