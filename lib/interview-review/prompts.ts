...
4. 每一题自动推荐 1~3 个分类标签（从内容中推断，如"项目深挖"、"行为面"、"案例分析"、"自我介绍"、"专业基础"、"场景题"、"数据分析"、"沟通表达"等，根据实际面试岗位类型选择合适标签）
...
8. **重要：根据面试内容、JD和简历判断候选人的实际岗位类型（研发/产品/设计/运营/销售/管理等），用该岗位的评价标准来点评，不要用研发的标准去要求产品经理，也不要用技术思维去评价运营岗**
...
7. 用通俗易懂的语言，避免堆砌专业术语
8. **重要：根据面试岗位类型调整改写风格——研发岗侧重技术深度和方案，产品岗侧重用户洞察和业务逻辑，运营岗侧重数据增长和执行力，设计岗侧重设计理念和用户体验，管理岗侧重团队协作和决策能力**
...
**重要：根据面试内容判断候选人的岗位类型，出题风格和考察维度要匹配该岗位——不要用研发的标准出题给产品经理，也不要用技术思维出题给运营岗。**
...
- 若"项目讲不清" → 出深挖项目细节的连续追问（你的角色？遇到什么困难？怎么解决的？成果怎样？）
- 若"量化不足" → 出需要数据支撑的"结果量化"追问
- 若"专业基础薄弱" → 出核心概念和原理解释题（技术岗考技术原理，产品岗考产品方法论，运营岗考数据指标等）
- 若"沟通表达差" → 出需要清晰表达复杂概念的题
- 若"缺少亮点" → 出能展现候选人优势和差异化的题
- 若"业务理解不深" → 出考察行业认知和商业思维的题
- 若"用户思维欠缺" → 出用户场景和需求分析的追问
...
9. 用通俗易懂的语言，不要堆砌专业术语
10. **重要：根据面试内容和JD判断实际岗位类型，用该岗位的评价标准来总结和打分——产品经理看用户洞察/需求分析/业务逻辑，运营看数据增长/活动策划/执行力，设计看审美/用户体验/设计思维，研发看技术深度/架构能力/代码质量，管理看团队协作/决策力/战略思维。不要用错评价维度！**
...
【参考标签池（不限于此，可以自由扩展，根据实际岗位类型选择合适标签）】
- 考察类型：项目深挖、结构化表达、业务理解、行为面（BQ）、专业基础、案例分析、场景题、反问质量
- 面试特征：压力面、轻松氛围、高难度、基础考察、文化匹配、薪资谈判
- 技术方向：前端、后端、全栈、移动端、算法、数据工程、测试、运维、架构、系统设计
- 产品/设计：产品设计、需求分析、用户体验、交互设计、竞品分析、数据指标、产品策略
- 运营/市场：用户增长、活动策划、内容运营、数据分析、渠道管理、品牌营销
- 管理/通用：团队管理、项目管理、战略思维、跨部门协作、领导力
- 其他：自我介绍、职业规划、离职原因
...
**重要：根据面试内容和岗位类型选择对应的评价维度——不要用研发的技术深度标准去评判产品经理，也不要用技术思维去评价运营岗。**
${jdHint}
【面试题】
${question}
...
**注意：根据面试岗位类型调整评估标准，不要用研发标准评价非研发岗位。**
${contextSection}
【追问题目】
${followUpQuestion}
...
2. 深度（depth）：是否展现了足够的专业深度或思考深度（根据岗位类型判断：研发看技术深度，产品看用户/业务洞察深度，运营看数据/策略深度等）
...
**重要：根据面试内容判断候选人的岗位类型，训练任务要匹配该岗位的能力要求——研发岗练技术深度，产品岗练需求分析和用户洞察，运营岗练数据分析和增长策略，设计岗练设计表达和作品集展示等。不要给非研发岗位布置技术类训练任务。**
...
5. task_type 分类：
   - practice：答题结构练习（分点论述、STAR法则、框架搭建）
   - review：专业知识复习（核心概念、方法论、行业知识等）
   - drill：模拟追问训练（连续深挖）
   - expression：表达优化（量化数据、简洁表述、故事化表达）
